...
const NumberInput = ({ handle, error, setError }: NumberInputPropsType) => {
  ...
  useDebounce(
    () => {
      ...
      value ? urlSearchParams.set(handle, Number(value).toFixed(0).toString()) : urlSearchParams.delete(handle);
  ...
  const handleInputChange = (key: string, value: string) => {
    ...
    if (key === MIN_PRICE_HANDLE && (compareValue && intValue && intValue >= compareValue)) {
      setError('Min price must be less than max price');
    } else if (key === MAX_PRICE_HANDLE && (compareValue && intValue && intValue <= compareValue)) {
  ...
  return (
    <div className="flex flex-col gap-2">
      <label>
        {handle === MIN_PRICE_HANDLE ? 'Min' : 'Max'}
      </label>
      <Input
        id={handle}
        type="number"
        ...
        value={value}
        onChange={({ currentTarget }) => handleInputChange(handle, currentTarget.value)}
      />
    </div>
  );
}
